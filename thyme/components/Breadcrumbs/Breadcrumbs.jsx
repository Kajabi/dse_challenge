import React from 'react';
import PropTypes from 'prop-types';
import classnames from 'classnames';
import { Icon } from '../Icon';
import { Link } from '../Link';
import { ThymeTokens } from '../configs';
import { breadcrumbItemPropTypes } from './configs';

export const Breadcrumbs = ({
  className,
  icon,
  isProgressbar,
  items,
  ...rest
}) => {
  const classNames = classnames(
    'thyme-breadcrumbs',
    className,
    {
      'thyme-breadcrumbs--progressbar': isProgressbar
    }
  );
  let currentSet = false;
  const renderedItems = items.map(({
    current,
    disabled,
    label,
    linkTag,
    ...otherProps
  }, i) => {
    const isCurrent = current || (!currentSet && items.length > 1 && i + 1 === items.length);
    currentSet = currentSet || isCurrent;

    const content = (
      <Link
        aria-current={isCurrent ? 'page' : false}
        aria-disabled={disabled}
        className={`thyme-breadcrumbs__link ${isCurrent && 'thyme-breadcrumbs__link--current'}`}
        tag={linkTag}
        {...otherProps}
      >
        {(icon && items.length === 1 && i === 0) && (
          <Icon className="thyme-breadcrumbs__icon" icon={icon} />
        )}
        {label}
      </Link>
    );

    return items.length === 1 ? (
      <p key={i.toString()} className="thyme-breadcrumbs__item">{content}</p>
    ) : (
      <li key={i.toString()} className="thyme-breadcrumbs__item">{content}</li>
    );
  });

  return (
    <nav
      aria-label="Breadcrumbs"
      className={classNames}
      {...rest}
    >
      {items.length > 1 ? (
        <ul className="thyme-breadcrumbs__list">
          {renderedItems}
        </ul>
      ) : (
        <>
          {renderedItems}
        </>
      )}
    </nav>
  );
};

Breadcrumbs.itemPropTypes = breadcrumbItemPropTypes;

Breadcrumbs.defaultProps = {
  className: null,
  icon: ThymeTokens.ICONS.CARET_LEFT,
  isProgressbar: false,
  items: [],
};

Breadcrumbs.propTypes = {
  className: PropTypes.string,
  icon: PropTypes.oneOf(Object.values(ThymeTokens.ICONS)),
  isProgressbar: PropTypes.bool,
  items: PropTypes.arrayOf(breadcrumbItemPropTypes),
};
