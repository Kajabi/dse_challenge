...
export const Description = ({
  ...
  const classNames = classnames(
    ...
    className,
    {
      'thyme-description--no-dividers': noDividers,
      [`thyme-description--${layout}`]: layout,
    }
  );
  ...
  const renderItem = ({ title, data, action }) => {
    ...
    return (
      <>
        {title && (
          <dt className="thyme-description__title">
            {title}
          </dt>
        )}
        {data && (
          <dd className="thyme-description__data">
            {data}
          </dd>
        )}
        {action && (
          <div className="thyme-description__action">
            <Button
              color={Button.COLORS.PRIMARY}
              subtle={true}
              icon={ThymeTokens.ICONS.CARET_RIGHT}
  ...
  const renderItems = () => {
    ...
    return items.map((item) => {
      ...
      const termGroupClassnames = classnames(
        ...
        {
          'thyme-description__term-group--no-action': !hasActions,
          'thyme-description__term-group--hide-title': item.hideTitle,
        }
      );
  ...
  const getCustomProps = () => {
    ...
    if (actionWidth) {
      props['--thyme-description-action-width'] = actionWidth;
    }

    if (titleWidth) {
      props['--thyme-description-title-width'] = titleWidth;
    }
